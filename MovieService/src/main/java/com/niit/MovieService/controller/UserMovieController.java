...
public class UserMovieController
{
    ...
    public ResponseEntity<?> deleteUserProductFromList(@PathVariable int movieId,@PathVariable String email, HttpServletRequest request)
            throws UserNotFoundException, MovieNotFoundException
    {
        try {
            return new ResponseEntity<>(userService.deleteUserMovieFromList(email,movieId), HttpStatus.OK);
