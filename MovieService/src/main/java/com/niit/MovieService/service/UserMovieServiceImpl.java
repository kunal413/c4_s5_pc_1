...
public class UserMovieServiceImpl implements UserMovieService {
    ...
    @Override
    public UserModel deleteUserMovieFromList(String email, int movieId) throws UserNotFoundException, MovieNotFoundException {
        boolean movieIdIsPresent = false;
        if(userMovieRepository.findById(email).isEmpty())
        {
            throw new UserNotFoundException();
        }
        UserModel user = userMovieRepository.findById(email).get();
        List<Movie> movies = user.getMovieList();
